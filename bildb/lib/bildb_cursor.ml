...
module Make (Machine : Primus.Machine.S) = struct
  ...
  let get_blk () : Blk.t option Machine.t =
    let* data = Machine.Global.get Data.data in
  ...
  let is_stopping_point (tid : Tid.t) : bool Machine.t =
    let* is_breakpoint = is_break tid in
    if is_breakpoint then Machine.return true
    else
      let* is_blk = is_block tid in
      if is_blk then Machine.return true
      else
        let* is_first = is_first_in_block tid in
        if is_first then Machine.return true
        else
          let* is_sub = is_subroutine tid in
  ...
  let is_initial_machine () : bool Machine.t =
    ...
    let* pid = Machine.ancestor [cid] in
  ...
  let update (tid : Tid.t) : unit Machine.t =
    let* prog = Machine.gets Project.program in
    let* () = match Program.parent def_t prog tid with
  ...
  let rec run_from (tid : Tid.t) : unit Machine.t =
    ...
    (* Spawn a new machine. *)
    let* pid = Machine.current () in
    let* _ = Machine.fork () in
    let* cid = Machine.current () in
      ...
      begin
        ...
        | false ->
          begin
            ...
            let* is_init = is_initial_machine () in
            match is_init with
            | true ->
              begin
                let* _ = set_forward () in
                let* _ = set_step_mode () in
                let* _ = mark_not_just_visited () in
                let* _ = mark_just_reached_start () in
                run_from tid
              end
            ...
            | false ->
              begin
                ...
                let* is_stop = is_stopping_point tid in
                let* just_visited = was_just_visited () in
