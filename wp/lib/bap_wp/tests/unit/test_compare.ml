...
let test_sub_pair_6 (test_ctx : test_ctxt) : unit =
  let ctx = Env.mk_ctx () in
  let var_gen = Env.mk_var_gen () in
  let tgt = Theory.Target.unknown in
  let env1 = Pre.mk_env ctx var_gen ~arch:tgt ~exp_conds:[Pre.non_null_load_assert] in
  let env2 = Pre.mk_env ctx var_gen ~arch:tgt ~exp_conds:[Pre.non_null_load_vc] in
