...
let test_nested_ifs (threshold : float) (test_ctx : test_ctxt) : unit =
      ...
      Seq.find_mapi (branch_info sub)
        ~f:(fun i (t, cond) ->
            ...
            if Tid.equal t tid then begin
              ...
              None)
    in
    let env = Pre.mk_env ctx var_gen ~arch:test_tgt ~jmp_spec ~subs:(Seq.singleton sub) in
  ...
  let time_without_hooks =
    ...
    let env = Pre.mk_env ctx var_gen ~arch:test_tgt ~subs:(Seq.of_list [sub; assert_sub])
