...
let exp_to_z3 (exp : Exp.t) (env : Env.t) : Constr.z3_expr * Constr.t list * Constr.t list * Env.t =
  ...
  let rec exp_to_z3_body exp env : Constr.z3_expr * Env.t =
    ...
    | BinOp (bop, x, y) ->
      ...
      (* Instructions such as left and right shifts do not require operands to
         have the same bitwidths, but Z3 does. Here, we pad the operand with the
         smaller bitwidth to match the bitwidth of the other operand. *)
      let x_val, y_val =
        match bop with
        | LSHIFT | RSHIFT | ARSHIFT ->
          let x_size = get_size x_val in
          let y_size = get_size y_val in
          (* Should we zero extend or sign extend these values? *)
