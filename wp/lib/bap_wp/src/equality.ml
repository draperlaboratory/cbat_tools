...
let get_label (a: Ir.Node.t) : blk term =
  Ir.Node.label a
...
let get_node_map (cfg1 : Ir.t) : (blk term) Tid.Map.t =
  let acc = fun ele tid_map ->
    ...
    Tid.Map.update tid_map tid ~f:(fun _ -> get_label ele) in
...
let exist_isomorphism (sub1: Sub.t) (sub2 : Sub.t) : (Tid.t Tid.Map.t) option =
  let cfg1, cfg2 = Sub.to_cfg sub1, Sub.to_cfg sub2 in
