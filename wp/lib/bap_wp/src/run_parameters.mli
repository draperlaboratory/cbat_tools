...
(** [validate_mem_flag mem_offset rewrite_addrs init_mem files] checks that a
    memory comparison flag is only set when there are two files to compare, and
    that only one flag is set at a time. Returns an error otherwise. *)
val validate_mem_flags :
  bool -> bool -> bool -> string list -> (unit, error) result
