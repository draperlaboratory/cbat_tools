...
let map_to_eqs (ctx : Z3.context) (m : var Var.Map.t) : Env.constr list =
  ...
  List.map pairs ~f:(fun (v1, v2) ->
      ...
      Bool.mk_eq ctx var1 var2 |> Env.mk_goal "eq" |> Env.mk_constr)
...
let compare_blocks
  ...
  let output_eq = Env.mk_clause [] (map_to_eqs ctx output_map) in
  ...
  let goal = Env.mk_clause input_eq [pre2] in
  goal, env2
...
type comparator = original:(Sub.t * Env.t) -> modified:(Sub.t * Env.t) ->
  rename_map:(var Var.Map.t) -> Env.constr
...
let compare_subs
  ...
  info "\nHypotheses:\n%s\n%!" (Env.constr_to_string hyps);
  let goal = Env.mk_clause [hyps] [pre_combined] in
  goal, env2
...
let compare_subs_empty
  ...
  let postcond ~original:(_, env) ~modified:_ ~rename_map:_ =
    ...
    Bool.mk_true ctx |> Env.mk_goal "true" |> Env.mk_constr
  in
...
let compare_subs_empty_post
  ...
  let hyps ~original:(_, env) ~modified:_ ~rename_map =
    ...
    in
    Env.mk_clause [] (map_to_eqs ctx input_map)
  in
  compare_subs ~postcond:postcond ~hyps:hyps ~original:original ~modified:modified
...
let compare_subs_eq
  ...
  let postcond ~original:(_, env) ~modified:_ ~rename_map =
    ...
    in
    Env.mk_clause [] (map_to_eqs ctx output_map)
  in
  let hyps ~original:(_, env) ~modified:_ ~rename_map =
    let ctx = Env.get_context env in
    let input_map = Var.Map.filteri rename_map
...
let compare_subs_fun
      ...
      Env.fold_fun_tids env1 ~init:[]
        ~f:(fun ~key:f ~data:_ goal ->
            let f_not_called_original =
              ...
              |> Env.mk_goal "not_called_original"
              |> Env.mk_constr
            in
            let f_not_called_modified =
              Bool.mk_not ctx (mk_is_fun_called env2 f)
              |> Env.mk_goal "not_called_modified"
              |> Env.mk_constr
            in
            let clause = Env.mk_clause [f_not_called_original] [f_not_called_modified] in
            clause::goal
          )
    in
    Env.mk_clause [] no_additional_calls
  in
  let hyps ~original:(_, env1) ~modified:(_, env2) ~rename_map =
    ...
    Env.mk_clause [] (eqs @ modified_not_called)
  in
  compare_subs ~postcond:postcond ~hyps:hyps ~original:original ~modified:modified
