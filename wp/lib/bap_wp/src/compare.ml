...
let map_to_eqs (ctx : Z3.context) (m : var Var.Map.t) : Env.z3_expr =
  let pairs = Var.Map.to_alist m in
  let eqs = List.map pairs ~f:(fun (v1, v2) ->
...
let compare_blocks
  ...
  let output_eq = map_to_eqs ctx output_map in
  ...
  let goal = Bool.mk_implies ctx input_eq pre2 in
  goal, env2
...
type comparator = original:(Sub.t * Env.t) -> modified:(Sub.t * Env.t) ->
  rename_map:(var Var.Map.t) -> Env.z3_expr
...
let compare_subs
  ...
  info "\nHypotheses:\n%s\n%!" (Expr.to_string hyps);
  (* This choice seems a bit arbitrary, but ctx needs to be shared
     between env1 and env2 anyway... *)
  let ctx = Env.get_context env1 in
  let goal = Bool.mk_implies ctx hyps pre_combined in
  goal, env2
...
let compare_subs_fun
  ...
  let postcond ~original:(_, env1) ~modified:(_, env2) ~rename_map:_ =
    ...
    let no_additional_calls =
      Env.fold_fun_tids env1 ~init:(Bool.mk_true ctx)
        ~f:(fun ~key:f ~data:_ goal ->
            let f_called_original = mk_is_fun_called env1 f in
            let f_called_modified = mk_is_fun_called env2 f in
            Bool.mk_and ctx [Bool.mk_implies ctx
                               (Bool.mk_not ctx f_called_original)
                               (Bool.mk_not ctx f_called_modified);
                             goal]
          )
    in
    no_additional_calls
  ...
  let hyps ~original:(_, env1) ~modified:(_, env2) ~rename_map =
    ...
    let modified_not_called =
      Env.fold_fun_tids env2 ~init:(Bool.mk_true ctx)
        ~f:(fun ~key:f ~data:_ hyp ->
            let f_called_modified = mk_is_fun_called env2 f in
            Bool.mk_and ctx [ Bool.mk_not ctx f_called_modified ; hyp ]
          )
    in
    let eqs = map_to_eqs ctx rename_map in
    Bool.mk_and ctx [eqs; modified_not_called]
  in
  compare_subs ~postcond:postcond ~hyps:hyps ~original:original ~modified:modified